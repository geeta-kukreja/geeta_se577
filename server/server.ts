...
async function getLastcommit(repoName: string) {
  ...
  if (commits.length == 0 || commits[0] == undefined) return "No commits found";
  else return commits[0].commit["message"];
}
...
server.get<requestQry>("/search", async (request, reply) => {
  ...
  repos = JSON.parse(response.payload); // repos = JSON.parse(repos)
  ...
  if (id) {
    ...
    if (repo) {
      ...
      let errObj = {
        error: `Repository = ${id} was not found with name = ${name} was not found`,
      };
      reply.code(404).send(errObj);
      return;
    }
    ...
    let errObj = {
      error: "The /search API requires an id or Repo name query parameter",
    };
...
server.get<requestQry>("/commits", async (request, reply) => {
  ...
  if (name) {
    ...
    let errObj = {
      error: "The /search API requires Repo name query parameter",
    };
    reply.code(400).send(errObj);
    return;
  }
...
server.post<RequestId>("/favgists", async (request, reply) => {
  try{
    const { name, URL } = request.body as RequestId;
    let updatedData = { name: name, URL: URL };
    return favgists.push(updatedData);
  }catch(e){
    let errObj = { error: "Cannot add to fav" };
    reply.code(404).send(errObj);
    return;
  }
});
...
server.get("/allFavGists", async (request, reply) => {
  return favgists;
});
